...
export default function ForkliftROICalculator() {
    const searchParams = useSearchParams();

    const [inputs, setInputs] = useState<CalculatorInputs>({
        numberOfForklifts: 10,
        shiftsPerDay: 2,
        daysPerWeek: 5,
        weeksPerYear: 52,
    ...
    // Initialize numberOfForklifts and electricityRate from URL query parameters
    useEffect(() => {
        const forkliftsFromUrl = searchParams.get('forklifts');
        const electricityRateFromUrl = searchParams.get('electric-rate');

        let updatedInputs = { ...inputs };

        if (forkliftsFromUrl) {
            const parsedForklifts = parseInt(forkliftsFromUrl, 10);
            if (!isNaN(parsedForklifts) && parsedForklifts > 0) {
                updatedInputs.numberOfForklifts = parsedForklifts;
            }
        }

        if (electricityRateFromUrl) {
            const parsedElectricityRate = parseFloat(electricityRateFromUrl);
            if (!isNaN(parsedElectricityRate) && parsedElectricityRate > 0) {
                updatedInputs.electricityRate = parsedElectricityRate;
            }
        }

        // Only update state if there were changes
        if (forkliftsFromUrl || electricityRateFromUrl) {
    ...
    const calculateResults = (): CalculatorResults => {
        ...
        const leadAcidInitialInvestment =
            numberOfForklifts * 2 * leadAcidBatteryCost; // 2 batteries per forklift
        const lithiumInitialInvestment =
            numberOfForklifts * 1 * lithiumBatteryCost; // 1 battery per forklift

        // 2. Battery Replacement Costs
        const leadAcidReplacementCost =
            numberOfForklifts * leadAcidBatteryCost * 0.5; // 50% replacement around year 3
        const lithiumReplacementCost = 0; // No replacement needed in 5 years
        ...
        const leadAcidEnergyCost = leadAcidAnnualKwh * electricityRate * 5; // 5-year period
        const lithiumEnergyCost = lithiumAnnualKwh * electricityRate * 5;

        // 4. Maintenance Costs
        const leadAcidMaintenanceCost = numberOfForklifts * 2 * 200 * 5; // 20 batteries * $200/year * 5 years
        const lithiumMaintenanceCost = 0; // No maintenance needed

        // 5. Labor Costs for Battery Swapping
        ...
        const leadAcidLaborCost = swapTimeHours * laborCost * 5; // 5-year period
        const lithiumLaborCost = 0; // No swapping needed
                ...
                <div className="rounded-lg bg-white p-8 shadow-lg">
                    ...
                    <div className="mb-8">
                        <h2 className="mb-6 text-2xl font-semibold text-neutral-800">
                            Input Variables/Assumptions (Adjust based on your
                            own numbers)
                        </h2>
                    ...
                    <div className="mb-8">
                        ...
                        <div className="mb-6 rounded-lg bg-neutral-50 p-6">
                            ...
                            <div className="space-y-3">
                                <p className="text-neutral-700">
                                    ...
                                    batteries per forklift to operate 2 shifts
                                    (one charging while the other is in use).
                                </p>
                                <p className="ml-4 text-neutral-700">
                                    {inputs.numberOfForklifts} forklifts × 2
                                    batteries/forklift ×{' '}
                                    {formatCurrency(inputs.leadAcidBatteryCost)}
                                    /battery ={' '}
                                    <strong>
                                        {formatCurrency(
                                ...
                                <p className="text-neutral-700">
                                    <strong>Lithium:</strong> You only need 1
                                    battery per forklift as it charges during
                                    breaks and between shifts.
                                </p>
                                <p className="ml-4 text-neutral-700">
                                    {inputs.numberOfForklifts} forklifts × 1
                                    battery/forklift ×{' '}
                                    {formatCurrency(inputs.lithiumBatteryCost)}
                                    /battery ={' '}
                                    <strong>
                                        {formatCurrency(
                        ...
                        <div className="mb-6 rounded-lg bg-neutral-50 p-6">
                            ...
                            <div className="space-y-3">
                                <p className="text-neutral-700">
                                    <strong>Lead-Acid:</strong> Lifespan is ~5
                                    years or 1,500 cycles. In our 2-shift model,
                                    they are cycled heavily and will likely need
                                    replacement once during this 5-year period.
                                    Let's say 50% of them need replacement.
                                </p>
                                <p className="ml-4 text-neutral-700">
                                    Replacement Cost ={' '}
                                    {inputs.numberOfForklifts} batteries ×{' '}
                                    {formatCurrency(inputs.leadAcidBatteryCost)}
                                    /battery ={' '}
                                    <strong>
                                        {formatCurrency(
                                ...
                                <p className="text-neutral-700">
                                    <strong>Lithium:</strong> Lifespan is ~5,000
                                    cycles. It will easily last the entire
                                    5-year period without replacement.
                                </p>
                                <p className="ml-4 text-neutral-700">
                                    Replacement Cost ={' '}
                                    <strong>
                        ...
                        <div className="mb-6 rounded-lg bg-neutral-50 p-6">
                            ...
                            <div className="space-y-3">
                                <p className="text-neutral-700">
                                    <strong>Lead-Acid:</strong> ~75% efficient.
                                    It uses more electricity to charge due to
                                    heat and gas generation.
                                </p>
                                <p className="ml-4 text-neutral-700">
                                    Annual kWh Usage = (
                                    {inputs.numberOfForklifts} forklifts ×{' '}
                                    {inputs.shiftsPerDay} shifts ×{' '}
                                    {inputs.forkliftPowerConsumption} kWh/shift
                                    × {inputs.daysPerWeek * inputs.weeksPerYear}{' '}
                                    days) ÷ 0.75 efficiency ={' '}
                                    {(
                                ...
                                <p className="ml-4 text-neutral-700">
                                    ...
                                    kWh/year ×{' '}
                                    {formatCurrency(inputs.electricityRate)}/kWh
                                    × 5 years ={' '}
                                    <strong>
                                        {formatCurrency(
                                ...
                                <p className="text-neutral-700">
                                    <strong>Lithium:</strong> More than 95%
                                    efficient, up to 99%
                                </p>
                                <p className="ml-4 text-neutral-700">
                                    Annual kWh Usage = (
                        ...
                        <div className="mb-6 rounded-lg bg-neutral-50 p-6">
                            ...
                            <div className="space-y-3">
                                <p className="text-neutral-700">
                                    ...
                                    watering and cleaning. Estimate ~$200 per
                                    battery per year in maintenance labour, and
                                    additives.
                                </p>
                                <p className="ml-4 text-neutral-700">
                        ...
                        <div className="mb-6 rounded-lg bg-neutral-50 p-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <p className="text-neutral-700">
                                    <strong>Lead-Acid:</strong> Each battery
                                    swap takes ~15 minutes (including travel
                                    to/from the charging room). 2 swaps per
                                    forklift per day.
                                </p>
                                <p className="ml-4 text-neutral-700">
                                    Annual Labour Hours ={' '}
                                ...
                                <p className="ml-4 text-neutral-700">
                                    ...
                                    hours/year ×{' '}
                                    {formatCurrency(inputs.laborCost)}/hour × 5
                                    years ={' '}
                                    <strong>
                                        {formatCurrency(
                                ...
                                <p className="text-neutral-700">
                                    <strong>Lithium:</strong> No battery
                                    swapping. The operator plugs in the truck
                                    themselves during a break (opportunity
                                    charging).
                                </p>
                                <p className="ml-4 text-neutral-700">
                                    5-Year Labour Cost ={' '}
                        ...
                        <div className="mb-8">
                            <h3 className="mb-4 text-xl font-semibold text-neutral-800">
                                ...
                                {inputs.numberOfForklifts} Forklifts: Lithium
                                vs. Lead-Acid
                            </h3>
                        ...
                        <div className="mb-6 rounded-lg border border-green-200 bg-green-50 p-6">
                            ...
                            <p className="text-green-700">
                                ...
                                and is paid back in about a month. The ongoing
                                savings are pure profit to invest in other areas
                                of your business.
                            </p>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="rounded-lg border border-blue-200 bg-gray-50 p-4">
                                ...
                                <p className="text-sm">
                                    Lead acid batteries require a dedicated room
                                    because the chemical reaction from charging
                                    and discharging lead acid batteries releases
                                    hydrogen gas from the electrolysis of
                                    distilled water. Sulfuric acid electrolyte
                                    requires careful handling due to corrosive
                                    and toxic nature. Lithium LFP batteries do
                                    not require this dedicated room and you can
                                    reclaim this valuable real-estate for
                                    storage or production. This could be an
                                    extra 1000s of square feet.
                                </p>
                            </div>
                            ...
                            <div className="rounded-lg border border-purple-200 bg-gray-50 p-4">
                                ...
                                <p className="text-sm">
                                    This is a hidden opportunity cost that is
                                    often missed by business planners. With lead
                                    acid batteries, a forklift is taken out of
                                    service for 15-20 minutes per shift for
                                    swapping. With lithium, it's a 15-second
                                    plug-in. This means more uptime and
                                    ...
                                    hours of forklift operation time. What is
                                    the value of that extra productivity for
                                    your business?
                                </p>
                            </div>
                            ...
                            <div className="rounded-lg border border-orange-200 bg-gray-50 p-4">
                                ...
                                <p className="text-sm">
                                    Reduced risk of injury from heavy battery
                                    swapping, acid spills, and hydrogen
                                    explosions. This can lower insurance
                                    premiums and costly accident lawsuits.
                                </p>
                            </div>
                            ...
                            <div className="rounded-lg border bg-gray-50 p-4">
                                ...
                                <p className="text-sm text-teal-700">
                                    Lithium provides full power until it's
                                    nearly empty, unlike lead-acid which
                                    steadily declines, leading to slower
                                    operation towards the end of the shift.
                                </p>
                            </div>
                        </div>
                    </div>
